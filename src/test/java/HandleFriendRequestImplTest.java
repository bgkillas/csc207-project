...
public class HandleFriendRequestImplTest {
    ...
    public void testHandleFriendRequest() {
        ...
        User user0 = new User("Stan", 20, "male", "Toronto", "I luv lord",
                emptyList, emptyList, emptyList);
        User user1 = new User("user01", 18, "female", "North York", "Let's go out!",
                emptyList, emptyList, emptyList);

        // Senario: user1 has been matched with user0 in her matching room and clicks "connect" button
        // which sends match request to user0. User0 finds this in his Match Request tab and also clicks "connect"

        HandleFriendRequest handleFriendRequest = new HandleFriendRequestImpl();

        UserSession userSession = new UserSession(user0);

        assertTrue(userSession.getIncomingMatches().isEmpty());  // user0's userSession has empty incoming matches
        assertTrue(userSession.getOutgoingMatches().isEmpty());  // user0's userSession has empty outgoing matches
