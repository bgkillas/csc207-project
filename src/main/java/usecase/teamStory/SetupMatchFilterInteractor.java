...
/**
 * This class implements the application logic for setting up a user's match filter
 * It takes the match preferences as input, creates a MatchFilter object,
 * attaches it to the currently logged-in user (from UserSession), and calls the presenter
 */
public class SetupMatchFilterInteractor implements SetupMatchFilterInputBoundary {
    ...
    public SetupMatchFilterInteractor(SetupMatchFilterOutputBoundary presenter,
                                      UserSession session) {
    ...
    public void setupFilter(int minAge, int maxAge, String preferredGender,
        ...
        MatchFilter filter = new MatchFilter(minAge, maxAge, preferredGender,
                preferredLocation);
